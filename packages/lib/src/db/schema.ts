...
export const scrAssetOriginEnum = selenaRegistrySchema.enum("asset_origin", ["UPLOADED", "GENERATED"]);
...
export const scrEditorialApprovals = selenaRegistrySchema
	.table(
		"editorial_approvals",
		{
			...
			reason: text("reason"),
			decidedBy: text("decided_by").notNull(),
			// Generated by the database. Two decisions written in one transaction
			// share a created_at, so the order they were written is the only thing
			// that can say which one stands.
			decidedSeq: bigint("decided_seq", { mode: "number" }).generatedAlwaysAsIdentity(),
			createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
		},
		(table) => ({
...
export const scrContentAssets = selenaRegistrySchema
	.table(
		"content_assets",
		{
			...
			origin: scrAssetOriginEnum("origin").notNull().default("UPLOADED"),
