...
| Только проверенные forward migrations | PASS | 34 миграции проверены дважды на чистой базе; повтор — no-op |
...
**Gate 5: BLOCKED по деплою, PASS по поведению продукта.** Развернуть в Railway
нельзя, пока нет доступа к репозиторию. Но существо требований — восемь
разделов, разделение runtime-ролей, ноль внешних вызовов — проверено запуском
на настоящей мигрированной схеме. Это другой класс доказательства, и он не
заменяет деплой: он доказывает, что разворачивать есть что.
