...
| Один результат создаёт ровно один Inbox item | PASS в БД, **NOT-VERIFIED сквозным путём** | Идемпотентность доказана на настоящем Postgres (`0035`-pgTAP: повтор → `duplicate`, устаревшая версия → `stale`, тот же id с другим содержимым → отказ). Сквозной прогон «задача в Aether → карточка в Control Room» не выполнялся |

**Gate 6: PASS по коду обеих сторон, NOT-VERIFIED сквозным путём.** Приёмник
развёрнут в staging и отвечает на healthcheck; проверка живого события требует
включить мост на Aether и прогнать одну задачу — это отдельное решение владельца,
и на проде оно означает отправку заголовков задач наружу.
...
| Событие из Aether появляется в Inbox ровно один раз | Частично. Обе стороны написаны и проверены тестами и на настоящем Postgres; приёмник поднят в staging. Сквозной прогон на живом контуре — **NOT-VERIFIED** |
