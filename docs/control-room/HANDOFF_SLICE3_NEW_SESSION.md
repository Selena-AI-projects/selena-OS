...
> **FINAL STATE, verified by the owner's screenshots at ~03:00 UTC on 2026-09-07.**
> Two earlier notes in this file contradicted each other; this one supersedes both.
>
> - `parkourcafe/selena-OS` **was transferred** to the organization and is now
>   **`Selena-AI-projects/selena-OS`** (same name). The old address redirects,
>   which is why `git` through the proxy kept answering and why an
>   "address stayed the same" impression is misleading — the browser's URL bar
>   shows the org.
> - The other product repository moved too: `Selena-AI-projects/selena-ai-visibility`
>   (PR #146 there is Blacksmith's own migration; unrelated to Content OS).
> - **The repository-level self-hosted runner survived the transfer.** The
>   Runners page of `Selena-AI-projects/selena-OS` lists `selena-remote-runner`
>   with labels `self-hosted, Linux, X64, selena-ci`, status **Idle**, after the
>   owner power-cycled the Hetzner box. **No re-registration is needed.** Ignore
>   the re-registration commands in §4a unless that page ever shows the runner
>   absent.
> - Blacksmith is connected to the organization (two Blacksmith runners Active
>   at the org level).
> - PR #32, branch `feat/content-os-slice3-creation`, head `3a6c9cd…` are all at
>   `Selena-AI-projects/selena-OS`. The `Build` re-run that was queued since
>   18:45 UTC may have been dropped by the transfer: the runner sat Idle with
>   nothing assigned. **First action for the new session: re-run `Build` on
>   `3a6c9cd` and watch the runner go Active.**
> - This session's GitHub tooling could not follow the repository across owners
>   (`add_repo` refuses cross-owner adds); a session opened on
>   `Selena-AI-projects/selena-OS` is required for PR/check-run work.
